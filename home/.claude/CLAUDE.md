...
- どうしても自身で解決できない問題に直面している場合は、依頼された作業を一度中断し、ユーザーへブロッカーとなっている内容を共有して協力を仰ぐこと
  ...
  - Bashツールの一部はSandbox上で実行されるため、ファイルアクセスやネットワーク接続がエラーになることがある
- セカンドオピニオンとして、`codex`スキル呼び出しによる第三者レビューを積極的に活用すること
  - モデルを指定する代わりに`--profile cc-review`を使うこと
  - 思考の深さは、プロフィール`cc-review`のデフォルト値である`high`以上を使うこと（計画は必ず`xhigh`）
  - サブエージェント上で実行せず、メインセッションで実行すること (関連: https://github.com/openai/codex-plugin-cc/issues/345)
